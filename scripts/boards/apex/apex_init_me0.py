...
def main():
    ...
    if config_param["watchdog_control"]:
        ...
        for chan in range(16):
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.GBTX_LINK_SELECT"), chan)
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.ADDRESS"), 0xED)
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.WRITE_DATA"), 0x63)
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.EXECUTE_WRITE"), 1)
        sleep(0.1)

    # Reset MGT channels
    for chan in range(12):
        write_reg(get_node("GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.TX_DIFF_CTRL" % chan), 0x18)
        write_reg(get_node("GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.TX_POLARITY" % chan), TX_INVERTION[chan])
        write_reg(get_node("GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.RX_POLARITY" % chan), RX_INVERTION[chan])
    ...
    if config_param["watchdog_control"]:
        ...
        for chan in range(16):
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.GBTX_LINK_SELECT"), chan)
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.ADDRESS"), 0xED)
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.WRITE_DATA"), 0x03)
            write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.EXECUTE_WRITE"), 1)
    ...
    if config_param["lpgbt_reset"]:
        ...
        for chan in range(16):
            if chan%2==0:
                write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.GBTX_LINK_SELECT"), chan)
                write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.ADDRESS"), 0x130)
                write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.WRITE_DATA"), 0xA3)
                write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.EXECUTE_WRITE"), 1)
                sleep(0.1)
                write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.ADDRESS"), 0x12F)
                write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.WRITE_DATA"), 0x80)
                write_reg(get_node("GEM_AMC.SLOW_CONTROL.IC.EXECUTE_WRITE"), 1)

    sleep(0.1)
    for vfat in range(0,24):
        if config_param["hdlc_addr"]:
            write_reg(get_node("GEM_AMC.GEM_SYSTEM.VFAT3.VFAT%d_HDLC_ADDRESS"%(vfat)), ME0_VFAT_HDLC_ADDRESSES[vfat])
        else:
            write_reg(get_node("GEM_AMC.GEM_SYSTEM.VFAT3.VFAT%d_HDLC_ADDRESS"%(vfat)), 0)
