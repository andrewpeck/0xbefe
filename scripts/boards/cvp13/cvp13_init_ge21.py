...
def main():
    ...
    for chan in range(16):
        # write_reg(get_node("BEFE.GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.QPLL0_RESET" % chan), 1)
        # write_reg(get_node("BEFE.GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.QPLL1_RESET" % chan), 1)
        write_reg(get_node("BEFE.GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.TX_DIFF_CTRL" % chan), 0x18)
        write_reg(get_node("BEFE.GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.TX_POLARITY" % chan), 0)
        write_reg(get_node("BEFE.GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.CTRL.RX_POLARITY" % chan), 1)
