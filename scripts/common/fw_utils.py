...
class MgtPll:
    ...
    def reset(self):
        if self.isqpll == 1:
            writeReg("BEFE.MGTS.MGT%d.CTRL.QPLL%d_RESET" % (self.idx, self.qpll01), 1)
        else:
            writeReg("BEFE.MGTS.MGT%d.CTRL.CPLL_RESET" % self.idx, 1)
...
class Mgt:
    ...
    def __init__(self, idx, txrx):
        ...
        self.type = readReg("BEFE.MGTS.MGT%d.CONFIG.LINK_TYPE" % self.idx)

    def get_status(self):
        reset_done = readReg("BEFE.MGTS.MGT%d.STATUS.%s_RESET_DONE" % (self.idx, self.txrx.name)).to_string(False)
        phalign_done = readReg("BEFE.MGTS.MGT%d.STATUS.%s_PHALIGN_DONE" % (self.idx, self.txrx.name)).to_string(False)
        pll_type = str(self.pll)
        pll_locked = self.pll.get_locked().to_string(False)
        refclk_freq = readReg("BEFE.MGTS.MGT%d.STATUS.REFCLK%d_FREQ" % (self.idx, self.pll.refclk01)).to_string(False)
    ...
    def config(self, invert, tx_diff_ctrl=0x18, tx_pre_cursor=0, tx_post_cursor=0):
        ...
        writeReg("BEFE.MGTS.MGT%d.CTRL.%s_POLARITY" % (self.idx, self.txrx.name), polarity)
        if self.txrx == MgtTxRx.TX:
            writeReg("BEFE.MGTS.MGT%d.CTRL.TX_DIFF_CTRL" % self.idx, tx_diff_ctrl)
            writeReg("BEFE.MGTS.MGT%d.CTRL.TX_PRE_CURSOR" % self.idx, tx_pre_cursor)
            writeReg("BEFE.MGTS.MGT%d.CTRL.TX_POST_CURSOR" % self.idx, tx_post_cursor)

    def reset(self, include_pll_reset=False):
        if include_pll_reset:
            self.pll.reset()
        writeReg("BEFE.MGTS.MGT%d.CTRL.%s_RESET" % (self.idx, self.txrx.name), 1)
...
class Link:
    ...
    def __init__(self, idx, tx_usage=None, rx_usage=None):
        self.idx = idx
        max_mgts = readReg("BEFE.SYSTEM.RELEASE.NUM_MGTS")
        tx_mgt_idx = readReg("BEFE.SYSTEM.LINK_CONFIG.LINK%d.TX_MGT_IDX" % self.idx)
        rx_mgt_idx = readReg("BEFE.SYSTEM.LINK_CONFIG.LINK%d.RX_MGT_IDX" % self.idx)
        self.tx_usage = tx_usage
        self.rx_usage = rx_usage
        if tx_mgt_idx < max_mgts:
            self.tx_mgt = Mgt(tx_mgt_idx, MgtTxRx.TX)
            self.tx_inv = True if readReg("BEFE.SYSTEM.LINK_CONFIG.LINK%d.TX_INVERTED" % self.idx) == 1 else False
        if rx_mgt_idx < max_mgts:
            self.rx_mgt = Mgt(rx_mgt_idx, MgtTxRx.RX)
            self.rx_inv = True if readReg("BEFE.SYSTEM.LINK_CONFIG.LINK%d.RX_INVERTED" % self.idx) == 1 else False
...
def befe_get_all_links():
    num_links = readReg("BEFE.SYSTEM.RELEASE.NUM_LINKS")

    # get the link usage (depends if it's CSC or GEM)
    tx_usage = ["NONE"] * num_links
    rx_usage = ["NONE"] * num_links
    flavor = readReg("BEFE.SYSTEM.RELEASE.FW_FLAVOR")
    if flavor == 0: # GEM
        pass
    elif flavor == 1:
        num_dmbs = readReg("BEFE.CSC_FED.CSC_SYSTEM.RELEASE.NUM_OF_DMBS")
        for i in range(num_dmbs):
            dmb_type = readReg("BEFE.CSC_FED.CSC_SYSTEM.RELEASE.DMB_LINK_CONFIG.DMB%d.TYPE" % i)
            dmb_label = "DMB%d (%s)" % (i, dmb_type.to_string(False))
            tx_link = readReg("BEFE.CSC_FED.CSC_SYSTEM.RELEASE.DMB_LINK_CONFIG.DMB%d.TX_LINK" % i)
            if tx_link < num_links:
                tx_usage[tx_link] = dmb_label
            num_rx_links = readReg("BEFE.CSC_FED.CSC_SYSTEM.RELEASE.DMB_LINK_CONFIG.DMB%d.NUM_RX_LINKS" % i)
            for j in range(num_rx_links):
                rx_link = readReg("BEFE.CSC_FED.CSC_SYSTEM.RELEASE.DMB_LINK_CONFIG.DMB%d.RX%d_LINK" % (i, j))
                if rx_link < num_links:
                    rx_usage[rx_link] = dmb_label
        # local DAQ link
        use_ldaq_link = readReg("BEFE.CSC_FED.CSC_SYSTEM.RELEASE.USE_LOCAL_DAQ_LINK")
        if use_ldaq_link != 0:
            ldaq_link = readReg("BEFE.CSC_FED.CSC_SYSTEM.RELEASE.LOCAL_DAQ_LINK")
            if ldaq_link < num_links:
                tx_usage[ldaq_link] = "LDAQ"
                rx_usage[ldaq_link] = "LDAQ"
    else:
        printRed("Unknown firmware flavor %s" % str(flavor))
...
def befe_reset_all_plls():
    num_mgts = readReg("BEFE.SYSTEM.RELEASE.NUM_MGTS")
    for i in range(num_mgts):
        writeReg("BEFE.MGTS.MGT%d.CTRL.CPLL_RESET" % i, 1)
        writeReg("BEFE.MGTS.MGT%d.CTRL.QPLL0_RESET" % i, 1)
        writeReg("BEFE.MGTS.MGT%d.CTRL.QPLL1_RESET" % i, 1)
...
def befe_print_fw_info():
    ...
    heading("BEFE %s v%s.%s.%s running on %s (built on %s at %s, git SHA: %08x)" % (flavor_str, fw_major.to_string(False), fw_minor.to_string(False), fw_build.to_string(False), board_type.to_string(False), date_str, time_str, fw_git_sha))
