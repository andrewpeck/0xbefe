...
def main():
    ...
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.CTRL.TTC_HARD_RESET_EN'), 1)
    ...
    for i in xrange(TEST_REG_ITERATIONS):
        ...
        write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.LOOPBACK.DATA'), val)
        loopVal = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.LOOPBACK.DATA'))
        if loopVal != val:
