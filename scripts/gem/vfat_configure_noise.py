...
def configureVfat(vfatN, ohN, threshold):
        ...
        for i in range(128):
            writeReg(getNode("GEM_AMC.OH.OH%i.GEB.VFAT%i.VFAT_CHANNELS.CHANNEL%i"%(ohN,vfatN,i)), 0x0000)  # unmask all channels
        ...
        writeReg(getNode("GEM_AMC.OH.OH%i.GEB.VFAT%i.CFG_THR_ARM_DAC"         % (ohN , vfatN)) , threshold)
...
def main():
    ...
    if threshold < 0 or threshold > 255:
        printRed("Invalid threshold, must be in the range of 0-255: %d" % threshold)
        return
    ...
    # unmask everything
    writeReg(getNode("GEM_AMC.OH.OH%i.FPGA.TRIG.CTRL.VFAT_MASK" % ohN), 0)
    for i in range(12):
        writeReg(getNode("GEM_AMC.OH.OH%i.FPGA.TRIG.CTRL.TU_MASK.VFAT%i_TU_MASK" % (ohN, i)), 0)
