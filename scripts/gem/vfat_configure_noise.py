...
def configureVfat(vfatN, ohN, threshold):
        ...
        for i in range(128):
            write_reg(get_node("GEM_AMC.OH.OH%i.GEB.VFAT%i.VFAT_CHANNELS.CHANNEL%i"%(ohN,vfatN,i)), 0x0000)  # unmask all channels
        ...
        write_reg(get_node("GEM_AMC.OH.OH%i.GEB.VFAT%i.CFG_THR_ARM_DAC"         % (ohN , vfatN)) , threshold)
...
def main():
    ...
    # unmask everything
    write_reg(get_node("GEM_AMC.OH.OH%i.FPGA.TRIG.CTRL.VFAT_MASK" % ohN), 0)
    for i in range(12):
