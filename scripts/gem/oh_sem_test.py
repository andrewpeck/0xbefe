...
def main():
    ...
    if DO_HARD_RESET:
        ...
        write_reg(get_node('GEM_AMC.TTC.GENERATOR.SINGLE_HARD_RESET'), 1)
        ...
        while init == 1 or obs == 0:
            init = read_reg(get_node("GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_INITIALIZATION"))
            obs = read_reg(get_node("GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_OBSERVATION"))
        print("=============== SEM IP is initialized and in OBSERVATION state ===============")

    addr = 0
#   if DO_SOFT_RESET:
#       print("entering idle state")
#       write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_ADDR_LSBS'), addr)
#       write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_ADDR_MSBS'), 0xe0) # enter idle state
#       write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_PULSE'), 1)
#       sleep(SLEEP)
#       print("applying soft reset to the SEM IP")
#       write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_ADDR_MSBS'), 0xb0) # soft reset
#       write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_PULSE'), 1)
...
def injectSemError(address, verbose=True, injectCritical=False, injectDoubleAdjacent=False):
    ...
    if verbose:
        print("entering idle state")
    write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_ADDR_MSBS'), 0xe0)
    write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_PULSE'), 1)
    idle = 0
    while idle == 0:
        idle = read_reg(get_node("GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_IDLE"))
    ...
    for addr in addresses:
        write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_ADDR_LSBS'), addr)
        write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_ADDR_MSBS'), 0xc0)
        write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_PULSE'), 1)
        inj = 1
        while inj == 1:
            inj = read_reg(get_node("GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_INJECTION"))

    # enter observation state
    if verbose:
        print("entering observation state")
    write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_ADDR_MSBS'), 0xa0)
    write_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.INJ_PULSE'), 1)
...
def readSemCounters(verbose=False):
    corrCnt = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.CNT_SEM_CORRECTION'))
    critCnt = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.CNT_SEM_CRITICAL'))
    if verbose:
...
def readSemStatus(verbose=False):
    init = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_INITIALIZATION'))
    obs = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_OBSERVATION'))
    corr = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_CORRECTION'))
    classif = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_CLASSIFICATION'))
    inj = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_INJECTION'))
    ess = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_ESSENTIAL'))
    uncorr = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_UNCORRECTABLE'))
    idle = read_reg(get_node('GEM_AMC.OH.OH0.FPGA.CONTROL.SEM.SEM_STATUS_IDLE'))
