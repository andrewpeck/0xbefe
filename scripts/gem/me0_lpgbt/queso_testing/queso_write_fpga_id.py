...
if __name__ == "__main__":
    ...
    for i in range(len(args.fpga)):
        ...
        print ("Writing ID (register 0x%02X) to FPGA %s = 0x%02X\n"%(fpga_reg_addr[args.fpga[i]]+0x01, args.fpga[i], int(args.id[i], 16)))
        ...
        # read fpga id
        print ("Reading ID (register 0x%02X) from FPGA %s\n"%(fpga_reg_addr[args.fpga[i]], args.fpga[i]))
