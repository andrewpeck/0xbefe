...
def initRegAddrs(vfat_idx):
    ...
    REG_CTP7_BOARD_ID = get_node('GEM_AMC.GEM_SYSTEM.BOARD_ID').address
    REG_LINK_ENABLE_MASK = get_node('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.LINK_ENABLE_MASK').address
    #REG_OH1_FW = get_node('GEM_AMC.OH.OH0.STATUS.FW.DATE').address
    #REG_OH1_VFAT_MASK = get_node('GEM_AMC.OH.OH0.CONTROL.VFAT.SBIT_MASK').address
    REG_VFAT_CONTROL_REG = get_node('GEM_AMC.OH.OH0.GEB.VFAT%d.VFAT_CHANNELS.CHANNEL0' % vfat_idx).address
...
def enableJtag(freqDiv=None):
    ...
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.ADC_MONITORING.MONITORING_OFF'), 0x1)
    sleep(0.01)
    subheading('Enable JTAG module')
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.JTAG.CTRL.ENABLE'), 0x1)
...
def disableJtag():
    ...
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.JTAG.CTRL.ENABLE'), 0x0)
...
def jtagCommand(restoreIdle, ir, irLen, dr, drLen, drRead):
    ...
    if drRead:
        ...
        tdi = read_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.JTAG.TDI'))
        ...
        if len > 32:
            ...
            tdi1 = read_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.JTAG.TDI'))
            tdi |= tdi1 << 32
            debug('tdi1 = ' + hex(tdi1))
            debug('tdi = ' + hex(tdi))
        ...
        if len > 64:
            ...
            tdi2 = read_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.JTAG.TDI'))
            tdi |= tdi2 << 64
            debug('tdi2 = ' + hex(tdi2))
            debug('tdi = ' + hex(tdi))
        ...
        if len > 96:
            ...
            tdi3 = read_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.JTAG.TDI'))
            tdi |= tdi3 << 96
            debug('tdi3 = ' + hex(tdi3))
            debug('tdi = ' + hex(tdi))
...
def sendScaCommand(sca_channel, sca_command, data_length, data, doRead):
    ...
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.SCA_CMD_CHANNEL'), sca_channel)
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.SCA_CMD_COMMAND'), sca_command)
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.SCA_CMD_LENGTH'), data_length)
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.SCA_CMD_DATA'), d)
    write_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.SCA_CMD_EXECUTE'), 0x1)
    reply = 0
    if doRead:
        reply = read_reg(get_node('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.SCA_RPY_DATA'))
    return reply
