...
#        regTest(REG_CTP7_BOARD_ID, 0xbeef, 0xffff, True, 2300000000)
        ...
        printRed("OHv3 not supported yet...")
        return
        subheading('Testing OH0')
        regTest(REG_OH1_FW, 0x20170302, 0xffffffff, False, 100000)
    elif instructions == 'vfat':
        if len(sys.argv) < 3:
            printRed("Please provide the VFAT number")
            return
        initRegAddrs(parseInt(sys.argv[2]))
        subheading('Testing OH0 VFAT #%d' % parseInt(sys.argv[2]))
...
def initRegAddrs(vfat_idx):
    ...
    REG_LINK_ENABLE_MASK = getNode('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.LINK_ENABLE_MASK').real_address
    #REG_OH1_FW = getNode('GEM_AMC.OH.OH0.STATUS.FW.DATE').real_address
    #REG_OH1_VFAT_MASK = getNode('GEM_AMC.OH.OH0.CONTROL.VFAT.SBIT_MASK').real_address
    REG_VFAT_CONTROL_REG = getNode('GEM_AMC.OH.OH0.GEB.VFAT%d.VFAT_CHANNELS.CHANNEL0' % vfat_idx).real_address
