...
def c2Mapping():
    ...
    for oh in range(4):
        ...
        sleep(0.1)
        for gbt in range(3):
            locked = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.PRBS_LOCKED" % gbt))
            wordCnt = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.MEGA_WORD_CNT" % gbt))
            errCnt = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.ERROR_CNT" % gbt))
            ...
            write_reg(get_node("BEFE.GEM_AMC.OPTICAL_LINKS.MGT_CHANNEL_%d.RESET.TX_RESET" % txMgtChan), 1)
            sleep(0.1)
            locked = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.PRBS_LOCKED" % gbt))
            wordCnt = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.MEGA_WORD_CNT" % gbt))
            errCnt = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.ERROR_CNT" % gbt))
...
def c6Mapping(txCableNum, rxCableNum):
    ...
    for i in range(len(txChannels)):
        ...
        write_reg(get_node("BEFE.GEM_AMC.GEM_SYSTEM.TESTS.GBT_LOOPBACK_EN"), 0)
        write_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.CTRL.OH_SELECT"), rxOh)
        write_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.CTRL.RESET"), 1)
        write_reg(get_node("BEFE.GEM_AMC.GEM_SYSTEM.TESTS.GBT_LOOPBACK_EN"), 1)
        sleep(0.1)

        locked = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.PRBS_LOCKED" % rxGbt))
        wordCnt = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.MEGA_WORD_CNT" % rxGbt))
        errCnt = read_reg(get_node("BEFE.GEM_AMC.GEM_TESTS.OH_LOOPBACK.GBT_%d.ELINK_0.ERROR_CNT" % rxGbt))
        # make sure there are no errors
        if (locked == 0) or (wordCnt < 3) or (errCnt > 0):
