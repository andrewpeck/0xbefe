...
architecture promless_arch of promless is
    ...
    signal loader_en_pipe_in        : std_logic_vector(g_NUM_CHANNELS - 1 downto 0);
    signal loader_en_pipe_out       : std_logic_vector(g_NUM_CHANNELS - 1 downto 0);
...
begin
    ...
    g_flatten_en : for i in 0 to g_NUM_CHANNELS - 1 generate
        loader_en_pipe_in(i) <= to_promless_i(i).en;
    end generate;
    
    i_en_pipe : entity work.pipe
        generic map(
            WIDTH     => g_NUM_CHANNELS,
            DEPTH     => g_INPUT_REG_STAGES,
            INFER_SRL => "no"
        )
        port map(
            clk_i  => loader_clk,
            data_i => loader_en_pipe_in,
            data_o => loader_en_pipe_out
        );
    ...
    begin
        if rising_edge(loader_clk) then
            loader_en_req <= or_reduce(loader_en_pipe_out);
        end if;
    end process;
    ...
    -- register the output
    -- instantiate a pipe for each output to aid routing to the various places it may need to go (most likely they all go to different SLRs)
    g_output_pipes : for i in 0 to g_NUM_CHANNELS - 1 generate
        signal from_promless_pipe_out   : std_logic_vector(12 downto 0);
    begin
        i_output_pipe : entity work.pipe
            generic map(
                WIDTH     => 13,
                DEPTH     => g_OUTPUT_REG_STAGES,
                INFER_SRL => "no"
            )
            port map(
                clk_i  => loader_clk,
                data_i => from_promless.error & from_promless.last & from_promless.first & from_promless.valid & from_promless.ready & from_promless.data,
                data_o => from_promless_pipe_out
            );

        from_promless_o(i).data <= from_promless_pipe_out(7 downto 0);
        from_promless_o(i).ready <= from_promless_pipe_out(8);
        from_promless_o(i).valid <= from_promless_pipe_out(9);
        from_promless_o(i).first <= from_promless_pipe_out(10);
        from_promless_o(i).last <= from_promless_pipe_out(11);
        from_promless_o(i).error <= from_promless_pipe_out(12);
